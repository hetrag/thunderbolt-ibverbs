...
symlinks on `PATH` (`usb4_ib_send_bw`, `usb4_ib_write_bw`, …), so it coexists
with Fedora's stock `perftest` rather than replacing it. Point the benchmark
runner at it with `TBV_PERFTEST=/usr/libexec/usb4-perftest`.

The kmod package `Conflicts` with `thunderbolt-ibverbs-dkms`: use the kmod
package when you pin a specific kernel, and DKMS when you track distro kernels.
